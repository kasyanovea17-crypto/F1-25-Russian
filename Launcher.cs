...
sealed class Launcher : Form {
 ...
 Launcher(bool testMode){
  ...
  Glyphs.Load(home);Text="F1 25 · Русский текст · 0.25";ClientSize=new Size(960,680);FormBorderStyle=FormBorderStyle.None;StartPosition=FormStartPosition.CenterScreen;AutoScaleMode=AutoScaleMode.None;BackColor=Color.Black;ForeColor=Color.Black;Font=new Font("Segoe UI",9.5f);DoubleBuffered=true;
  ...
  LabelAt(this,"ПОМОЩЬ",38,386,171,20,8,FontStyle.Bold,Color.FromArgb(156,163,165));
  Nav("help","Инструкция",0xf194,413);Nav("log","Журнал",0xf444,461);Nav("about","О проекте",0xf431,509);
  ...
  LabelAt(tip,"ОБНОВЛЕНИЯ STEAM",20,18,233,25,9,FontStyle.Bold,Color.Black);
  ...
  LabelAt(this,"Лаунчер 0.25",653,612,266,22,9,FontStyle.Bold,Color.Black);
 ...
 void BuildVoice(){var p=Page("voice","Язык озвучки");var c=new Card{Bounds=new Rectangle(0,51,400,369)};p.Controls.Add(c);
  ...
  voiceSteam=ButtonAt(c,"Открыть выбор языка в Steam",20,166,359,35,true);voiceSteam.Click+=(s,e)=>{
   if(Process.GetProcessesByName("F1_25").Length>0){SetStatus("Сначала закройте игру","После выхода из F1 25 откройте язык в Steam.",accent);return;}
 ...
 protected override void Dispose(bool disposing){if(disposing){contactTips.Dispose();if(backdrop!=null)backdrop.Dispose();}base.Dispose(disposing);}
  ...
  if(code==0){
   ...
   else if(action=="install"){prepared=true;SetStatus("Перевод установлен","Готово. Озвучка — по языку Steam. Можно запускать игру.",green);}
 ...
 void SelfTest(){
  ...
  Check(Text.Contains("0.25")&&TextUpdate.LauncherVersion=="0.25","launcher version");
